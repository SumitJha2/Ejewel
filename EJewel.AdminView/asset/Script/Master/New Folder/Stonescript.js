...
function loadStoneCut(category) {
    EJewel.AdminView.Services.AdminServices.GetStoneCut(category, 0,
    function (result) {
        $("#ddlCut").empty();
        $.each(result, function (key, value) {
...
function loadStoneShape(category) {
    EJewel.AdminView.Services.AdminServices.GetStoneShape(category, 0,
    function (result) {
        $("#ddlShape").empty();
        $.each(result, function (key, value) {
...
function loadStoneColor(category) {
    EJewel.AdminView.Services.AdminServices.GetStoneColor(category, 0,
    function (result) {
        $("#ddlColor").empty();
        $.each(result, function (key, value) {
...
function loadStoneType(category) {
    EJewel.AdminView.Services.AdminServices.GetStoneType(category, 0,
    function (result) {
        $("#ddlType").empty();
        $.each(result, function (key, value) {
...
function saveUpdateStone() {
    ...
    try {
        ...
        EJewel.AdminView.Services.AdminServices.SaveUpdateCenterStone(model,
        function (result) {
            hide_seek('loader', false);
            displayMsg('S', 'Stone Details Added Successfully');
            $("#btnReset").click();
            pageScroll('loader');
        },
        OnServiceError);
