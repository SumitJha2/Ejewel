...
function show_center_stone_details(ContentID, model) {
    $("#" + ContentID).empty();
    var table = '<table class="table table-bordered table-striped"><tbody id="tblCenterStoneDetails"></tbody></table>'
    $("#" + ContentID).append(table);
    ContentID = "tblCenterStoneDetails";
    //row SKU
    var row = create_row();
    var cell_l = create_cell(row, 'SKU #', [['align', 'center']]);
    var cell_r = create_cell(row, model.SKU, [['align', 'center']]);
    add_row(ContentID, row);
    //row shape
    var row = create_row();
    var cell_l = create_cell(row, 'Shape', [['align', 'center']]);
    var cell_r = create_cell(row, model.StoneShape, [['align', 'center']]);
    add_row(ContentID, row);
    //weight
    row = create_row();
    cell_l = create_cell(row, 'Weight', [['align', 'center']]);
    cell_r = create_cell(row, model.StoneCarate, [['align', 'center']]);
    add_row(ContentID, row);
    //StoneClarity
    row = create_row();
    cell_l = create_cell(row, 'Clarity', [['align', 'center']]);
    cell_r = create_cell(row, model.StoneClarity, [['align', 'center']]);
    add_row(ContentID, row);
    //StoneColor
    row = create_row();
    cell_l = create_cell(row, 'Color', [['align', 'center']]);
    cell_r = create_cell(row, model.StoneColor, [['align', 'center']]);
    add_row(ContentID, row);
    //StoneCut
    row = create_row();
    cell_l = create_cell(row, 'Cut', [['align', 'center']]);
    cell_r = create_cell(row, model.StoneCut, [['align', 'center']]);
    add_row(ContentID, row);
    //Stone price
    row = create_row();
    cell_l = create_cell(row, 'Cut', [['align', 'center']]);
    cell_r = create_cell(row, '$' + model.StonePrice, [['align', 'center']]);
    add_row(ContentID, row);
    //Stone Certification
    row = create_row();
    cell_l = create_cell(row, 'Certification', [['align', 'center']]);
    cell_r = create_cell(row, model.CertificateCertificationLab, [['align', 'center']]);
    add_row(ContentID, row);
}
